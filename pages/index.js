...
export default function MissingBabygirl() {
  ...
  const [loveNotes, setLoveNotes] = useState([
    ...
    "I have missed you so much lately and I am so upset that I can't show you my love often because of the distance. I feel deeply for you than I ever have and if I could just get one day with you soon, I am afraid I might kidnap you..."
  ]);
  ...
  return (
    <div className="min-h-screen bg-pink-100 flex flex-col items-center p-6">
      ...
      <p className="text-lg text-black mb-6 max-w-3xl text-center" style={{ fontFamily: 'Dancing Script, cursive' }}>
        Welcome to our private little corner of the internet. The distance is brutal sometimes, and I’m really starting to think of kidnapping you in my WagonR — but this wait actually deepens our love. And if we go deep enough (shhh), we’ll know it’s real. And if it’s real, you’ll find yourself taking me home in your hot car one day.
        <br/><br/>
        Will be worth the wait, right?
        <br/><br/>
        This is a reminder of what we’re fighting for (a five-hour-long makeout session) and a digital hug for when you really miss babygirl and feel mad that she’s not right here.
      </p>

      {/* Where is Babygirl? */}
      <div className="mb-6 w-full max-w-3xl bg-cyan-100 rounded-2xl shadow-lg p-6">
      ...
      <div className="mb-6 w-full max-w-3xl bg-pink-100 rounded-2xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          {galleryImages.map((src, idx) => (
            <motion.img
              ...
              src={src}
              alt="Babygirl"
              className="w-full h-64 object-cover rounded-xl shadow-lg hover:scale-105 transition-transform duration-300 cursor-pointer"
            />
          ))}
        </div>
        <input
      ...
      {/* Love Notes Section */}
      <div className="mb-6 w-full max-w-3xl bg-cyan-100 rounded-2xl shadow-lg p-6">
        <h2 className="text-3xl font-semibold mb-4">📝 Awwh, Babygirl</h2>
        <ul>
          {loveNotes.map((note, idx) => (
            <li key={idx} className="mb-4 bg-white p-4 rounded-xl shadow-lg">{note}</li>
          ))}
        </ul>
        <input
          type="text"
          ...
          placeholder="Add a new love note..."
          className="w-full p-4 mt-4 bg-cyan-50 rounded-xl shadow-lg text-black focus:outline-none focus:ring-2 focus:ring-pink-300"
        />
        <button onClick={handleAddNote} className="mt-4 bg-red-400 text-white py-2 px-4 rounded-full shadow-lg hover:bg-red-500 transition-all">
          Add Note
        </button>
      </div>
    </div>
